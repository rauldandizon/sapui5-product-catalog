...
Built by working through all ten stages of a guided SAPUI5 tutorial, then migrating the finished app from static mock data to a real OData V2 service — which surfaced (and required fixing) a handful of issues the tutorial itself didn't fully cover: OData's canonical entity-key URL syntax (`/Products(1)`, not array-index paths), filtering across a navigation property (`Category/CategoryName`), and setting up a working local CORS proxy.
